var FormElements = function() {"use strict";
    ...
    // Ajax functions container
    var runAjax = function() {

        function setFormValues(json) {
            $('#field_begin_km').val(json.km_start);
            $('#field_end_km').val(json.km_start);
        }

        $('body').on('change', '#field_car', function(e) {
            e.preventDefault();

            var car_number = this.options[this.selectedIndex].innerHTML;

            $.blockUI({
                message : '<i class="fa fa-spinner fa-spin"></i>Fetching car data.<br> please wait..'
            });
            $.ajax({
                url : 'getcardata.php',
                type : 'get',
